...
- Layout: four across on desktop; two by two at 1100px and below; one column at 768px and below. Cards are equal height with the call to action pinned to the foot. Decorative arrows hidden once cards wrap.
- Accessibility: arrows and decorative icons carry aria-hidden; arrow glyphs inside link text hidden from screen readers; a visually hidden h2 gives the row a heading so the outline no longer jumps h1 to h3; chips, primary and ghost buttons moved from flat teal (white on #2D8C6F is about 4.1:1, below AA for small text) to deep teal #1F6B53; visible focus ring for keyboard users; the buy link's accessible name says what it buys, the price and that checkout is on Payhip, starting with the visible text (WCAG 2.5.3).
- Verifier verify_products_packs.py v2, 41 checks across both pages: RED 25/41 before, GREEN 41/41 after; mutations (an arrow exposed to screen readers; chip back on flat teal) each seen RED. A verifier bug found and fixed on the way (it anchored on an HTML comment it strips first); the fixed verifier re-confirmed RED on the untouched page.
- Not fixed here, logged: the Admin Core tile image says "aligned with ISO 42001", which the 22 Sep ruling excludes; it needs a regenerated tile. White-on-flat-teal buttons exist on other pages; a site-wide contrast sweep is due.
...
- mission.html: title, descriptions, subtitle and the training section line moved to the buy side. press.html: descriptions moved to the buy side.
- Verifier /opt/qcai/scripts/website/verify_seo_products.py written first (PD18), RED 11/34 on the live tree, GREEN 34/34 after. verify_thought_fs.py still GREEN 28/28.
...
- New "What this makes possible" three-column section: the organisation flourishes (capacity without headcount), the people are amplified (judgement over drudgery, own the capability not a contract), the wider ecosystem grows (profits fund free post-AI training).
- Copy rules held: buy side, post-AI training, no sell words, no ISO or certification claim (OECD referenced as a standard we draw on, not a certificate held), hedged regulator wording unchanged, no client names, no em dashes, company numbers only.
- Verifier verify_hai_operating_model.py extended to 43 checks (OECD citation, the human-agency quote, the new section, the ecosystem language); seen RED 39/43 on the prior head, GREEN 43/43 after (log hai_operating_model_2026-09-17_155706.log).

## 2026-09-17 [C] Human-AI Operating Model page, white paper request form, Resources card
- New page human-ai-operating-model.html: the three layer model (your people, the approval gate, AI colleagues) with the 1120x630 muted autoplay loop QC_OperatingModel_Exploded_Faces_web.mp4 and its poster, the consequence test, the threshold model, and the white paper request form. Built on the gated-autonomy-white-paper.html pattern: FormSubmit ajax to information@ with subject "White paper request: Human-AI Operating Model", best-effort HubSpot capture, personal reply with QC_HumanAI_OperatingModel_WhitePaper_Ed1.pdf. The PDF is not served from the site.
- Three web assets added on dev through the GitHub upload page (e0847df) and the page through the same route (548f60b), because the Spark MCP has no binary transfer and the anonymous upload route is an open security finding. Page metadata, ld+json and social tags point at this page, not advisory.html; og:image is the poster. Missing .ac-model and .ac-blocks CSS and the qc-faces.js script were added so the three blocks and the faces ribbon render as on admincore.html.
- resources.html: fifth resource card after Gated Autonomy, "WHITE PAPER · HOW WE RUN AI", linking the page. mission.html: one link under the products line. sitemap.xml: page added, lastmod 2026-09-17.
- Copy rules held: buy side, post-AI training, no sell family words on the new page, no ISO or certification claim, "designed to support what your sector's regulator expects", no client names, no em dashes, no mechanism detail beyond the 4 August posts, company numbers only, Pexels credit in the footer.
- Verifier on the Spark: verify_hai_operating_model.py written first (PD18), RED 10/39 on the untouched checkout 16 September, GREEN 39/39 after. Logs in /opt/qcai/scripts/website/test_results/.
